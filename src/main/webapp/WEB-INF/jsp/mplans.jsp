...
<div><button class="form_button" onclick="additem();" type="button">Add Item</button> </div>
  <div class="row">
    ...
    <div class="navbar col-sm-4 text-center">
     ...
     <form method="POST"   id="<c:out value="${plan.planid}"/>" action="/savePlan" modelAttribute="plandetail">
     <input type="hidden" name="planid" value="<c:out value="${plan.planid}"/>">
      <div class="d-flex w-100 justify-content-between">
   	 <label>Plan Name:</label>
     <input type="text" placeholder="Enter plan name" value="<c:out value="${plan.name}"/>" name="name" required>   
   	 </div> 
     <div class="d-flex w-100 justify-content-between">
   	 <label>Number of Meals:</label>
     <input type="number" placeholder="Enter number of meals" value="<c:out value="${plan.nmeals}"/>" name="nmeals" required> 
   	 </div> 
   	 <div class="d-flex w-100 justify-content-between">
   	 <label>Price:</label>
     <input type="text" placeholder="Enter price" value="<c:out value="${plan.price}"/>" name="price" required> 
   	 </div>
     <div class="d-flex w-100 justify-content-between">
   	 <label>Description:</label>
     <input type="text" placeholder="Enter Description" value="<c:out value="${plan.desc}"/>" name="desc" required> 
   	 </div> 
   	 <div class="d-flex w-100 justify-content-between">
   	 <label>Validity:</label>
     <input type="number" placeholder="Enter no of valid days" value="<c:out value="${plan.validity}"/>" name="validity" required>
	 ...
	 var ele=document.getElementById("<c:out value='${plan.planid}'/>").elements;
	 for (let i = 0; i < ele.length; i++) { 
		  if (ele[i].id === "ena") {
			  <c:choose>
			  <c:when test="${plan.enable eq 'enable'}">
			  ele[i].checked=true;
			  </c:when>  
			</c:choose> 
		  } 
		  if (ele[i].id === "disa") {
			  <c:choose>
			  <c:when test="${plan.enable eq 'disable'}">
			  ele[i].checked=true;
			  </c:when>  
			</c:choose> 
		  } 
...
function additem(){
	...
	node.innerHTML='<form method="POST"    action="/savePlan" modelAttribute="plandetail">'+
	'<input type="hidden" name="planid" value="new">'+
	'<div class="d-flex w-100 justify-content-between">'+
	'<label>Plan Name:</label>'+
  	'<input type="text" placeholder="Enter plan name" name="name" required>  '+
    '</div> <div class="d-flex w-100 justify-content-between">'+
  	'<label>Number of Meals:</label>'+
  	'<input type="number" placeholder="Enter number of meals" name="nmeals" required> '+
  	'</div> <div class="d-flex w-100 justify-content-between">  <label>Price:</label>'+
  	'<input type="text" placeholder="Enter price" name="price" required> '+
  	'</div> <div class="d-flex w-100 justify-content-between">'+
  	'<label>Description:</label> <input type="text" placeholder="Enter Description" name="desc" required> '+
  	'</div><div class="d-flex w-100 justify-content-between"> <label>Validity:</label>'+
  	'<input type="number" placeholder="Enter no of valid days" name="validity" required>'+
	...
	'</form>'; 
	document.getElementById("planlist").prepend(node);
}
