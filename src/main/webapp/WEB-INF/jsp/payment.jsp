...
<nav class="navbar navbar-expand-lg sticky-top navbar-light" style="background-color: #e3f2fd;">
 ...
 <c:choose>
  <c:when test="${not empty cart}">
		...
		<div>tax:<c:out value="${tax}"/> </div>
		<div>total price:<c:out value="${TotalPrice}"/> </div>
  		<div id="paypal-button-container" style="align:center;margin-left:0px;margin-right:0px;"></div> 
  </c:when> 
  <c:otherwise> 
  </c:otherwise>
</c:choose>  
              ...
              // finalize the transaction
             	onApprove: (data, actions) => {
                  const captureOrderHandler = (details) => {
                      ...
                      //console.log('json:'+JSON.stringify(datails));
